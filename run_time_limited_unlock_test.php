...
if (! $enrollment) {
    echo json_encode(['error' => 'user not enrolled']).PHP_EOL;
    exit(1);
}

$course = Course::with(['modules' => fn ($q) => $q->with('lessons')->orderBy('order')])->find($enrollment->course_id);
if (! $course) {
    echo json_encode(['error' => 'course not found']).PHP_EOL;
    exit(1);
}
...
if (! $mid) {
    ...
    if (! $mod) {
        echo json_encode(['error' => 'no modules']).PHP_EOL;
        exit(1);
    }
    $mid = $mod->id;
}
...
$manualUnlockedModuleIds = DB::table('module_user')
    ...
    ->where('unlocked', true)
    ->where(function ($q) {
        $q->whereNull('unlocked_until')->orWhere('unlocked_until', '>', now());
    })
    ->pluck('module_id')
    ->map(fn ($id) => (string) $id)
    ->toArray();
